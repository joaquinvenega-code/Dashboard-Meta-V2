...
export async function getAdAccounts(): Promise<AdAccount[]> {
  const accounts = await fetchAllPages('/me/adaccounts', {
    fields: 'id,name,account_id,account_status,currency,balance,account_type,spend_cap,amount_spent,funding_source_details{id,type,display_string},prepaid_balance,is_prepaid_account,extended_credit_invoice_group',
    limit: 100,
  });
  return accounts;
}
