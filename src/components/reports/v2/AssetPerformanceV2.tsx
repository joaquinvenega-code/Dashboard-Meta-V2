        ...
        {sortedAssets.map((ad, index) => (
          <div key={ad.id} className="relative group flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-blue-300 transition-all shadow-sm">
            ...
            <div className="p-3 space-y-3 flex-1">
              <h4 className="text-[10px] font-bold text-slate-800 line-clamp-1 uppercase tracking-tight">{ad.name}</h4>
              
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1">
                    <Target className="w-2.5 h-2.5 text-blue-500" />
                    <span className="text-[8px] font-black text-slate-400 uppercase">ROAS</span>
                  </div>
                  <p className="text-[11px] font-black text-slate-900">{ad.roas.toFixed(2)}x</p>
                </div>
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1">
                    <ShoppingBag className="w-2.5 h-2.5 text-emerald-500" />
                    <span className="text-[8px] font-black text-slate-400 uppercase">Ventas</span>
                  </div>
                  <p className="text-[11px] font-black text-slate-900">{ad.purchases}</p>
                </div>
              </div>

              <div className="pt-2 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">Facturación</span>
                  <span className="text-[10px] font-black text-slate-900">{formatCurrency(ad.revenue, 'ARS')}</span>
                </div>
              </div>
            </div>
          </div>
